...
class SourceIndex(PackageConfig.RegistryModel, table=True):
    """数据库对象模型"""

    __tablename__ = "source_index"
    __table_args__ = {"schema": PackageConfig.SchemaTable, "comment": "表索引"}
    # 仅用于内部处理
...
class SourceIndexCreate(SQLModel):
    """前端创建模型 - 用于接口请求"""
...
class SourceIndexUpdate(SQLModel):
    """前端更新模型 - 用于接口请求"""

    source_index_id: Optional[int] = Field(default=None, sa_type=BigInteger, description="主键")
    source_table_id: Optional[int] = Field(default=None, sa_type=BigInteger, description="主键")
    name: Optional[str] = Field(default=None, max_length=255, description="索引名称")
    column_names: Optional[str] = Field(default=None, description="字段集合")
