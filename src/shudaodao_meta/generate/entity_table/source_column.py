...
# @Software ：PyCharm
# @Desc     ：SQLModel classes for shudaodao_meta.source_column
...
class SourceColumn(MetaConfig.RegistryModel, table=True):
    ...
    autoincrement: Optional[bool] = Field(
        default=None, sa_type=Boolean, nullable=True, description="启用自增"
    )
    ...
    # 反向关系 - 父对象
    SourceTable: "SourceTable" = Relationship(back_populates="SourceColumns")
    # 反向关系 - 父对象
    SourceView: "SourceView" = Relationship(back_populates="SourceColumns")


class SourceColumnCreate(SQLModel):
    """前端创建模型 - 用于接口请求"""
...
class SourceColumnUpdate(SQLModel):
    ...
    source_column_id: Optional[int] = Field(default=None, sa_type=BigInteger, description="主键")
    source_table_id: Optional[int] = Field(default=None, sa_type=BigInteger, description="主键")
    source_view_id: Optional[int] = Field(default=None, sa_type=BigInteger, description="主键")
    ...
    precision: Optional[int] = Field(default=None, description="总位数")
    type: Optional[str] = Field(default=None, max_length=20, description="database类型")
    primary_key: Optional[bool] = Field(default=None, description="是否主键")
    scale: Optional[int] = Field(default=None, description="小数位数")
