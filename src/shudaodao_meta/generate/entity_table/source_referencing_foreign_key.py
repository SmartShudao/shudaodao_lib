...
class SourceReferencingForeignKey(MetaConfig.RegistryModel, table=True):
    """数据库对象模型"""
    ...
    source_foreign_key_id: int = Field(
        foreign_key=f"{MetaConfig.SchemaForeignKey}source_foreign_key.source_foreign_key_id",
        ondelete="CASCADE",
        sa_type=BigInteger,
        description="主键",
    )
