...
# @Software ：PyCharm
# @Desc     ：SQLModel classes for shudaodao_meta.gen_view
...
class GenView(RegistryModel, table=True):
    """ 数据库对象模型 """
    __tablename__ = "gen_view"
    ...
    # 反向关系 -> 父对象
    Schema: "GenSchema" = Relationship(back_populates="GenViews")
    # 正向关系 -> 子对象
...
class GenViewCreate(GenViewBase):
    """ 前端创建模型 - 用于接口请求 """
    ...


class GenViewUpdate(GenViewBase):
    """ 前端更新模型 - 用于接口请求 """
    ...


class GenViewResponse(BaseResponse):
    """ 前端响应模型 - 用于接口响应 """
