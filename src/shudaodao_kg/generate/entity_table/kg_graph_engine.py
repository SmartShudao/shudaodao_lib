...
# @Software ：PyCharm
# @Desc     ：SQLModel classes for shudaodao_meta.kg_graph_engine
...
class KgGraphEngine(PackageConfig.RegistryModel, table=True):
    """数据库对象模型"""

    __tablename__ = "kg_graph_engine"
    __table_args__ = {"schema": PackageConfig.SchemaTable, "comment": "图数据库表"}
    # 仅用于内部处理
    __database_schema__ = PackageConfig.SchemaName
    __primary_key__ = ["graph_engine_id"]

    graph_engine_id: int = Field(
        default_factory=get_primary_id, primary_key=True, sa_type=BigInteger, description="主键"
    )
    graph_engine_name: str = Field(max_length=50, description="图数据库名称")
